...
def load_model(model_path, model):
    trained = torch.load(model_path, map_location=torch.device(config.DEVICE))
...
def evaluate (model,val_loader,batch_size,thresholds,get_percent=True):
    ...
    for thresh in tqdm(thresholds):
        for X, y_bin in tqdm(val_loader):
...
if __name__ == '__main__':
    ...
    model_path = config.MODEL_SAVE_PATH + f'/best_model_HotspotPredictor_{optim_name}_{config.LR}_{config.TRAIN_BATCH_SIZE}_{config.CLASS_THRESH}_{config.RANDOM_SEED}_{config.N_EPOCHS}.pt'
    ...
    model = HotspotPredictor(input_dim=len(config.CRIME_CATS), hidden_dim=64, kernel_size=config.KERNEL_SIZE)
    model.to(device)
